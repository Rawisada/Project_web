...
const BanListUserComponent=()=>{
    const[users, setUser] = useState([])

    const fetchData=()=>{
        axios.get(`${process.env.REACT_APP_API}/getUserEnable/false`)
        .then(response=>{
    ...
    return(
        <div className="container p-3">
            ...
            <a href="/listAllUser" className="btn btn-success function"> list of user</a>
            <table class="table table-hover" >
                ...
                {users.map((user,index)=>(
                        <tr  key={index} >
                            ...
                            <td>
                                <button className="btn btn-secondary function" onClick={()=>changeEnable(this,user.email, true)} >UNBAN</button>
...
export default BanListUserComponent;