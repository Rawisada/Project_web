...
const ListAllCactusComponentByLatest=(props)=>{
    ...
    const fetchData=()=>{
        axios.get(`${process.env.REACT_APP_API}/cactusByDateLatestToOldest/${props.match.params.word}/${props.match.params.type}`)
        .then(response=>{
    ...
    function myFuntionSearch(element,words){
        ...
        if(words == "" && type == "" ){
            history.push('/cactusAllByLatest/-/-')
            window.location.reload()
        }else if(words == "" && type == "-"){
            history.push('/cactusAllByLatest/-/-')
            window.location.reload()
        }else if(words == ""){
            history.push(`/cactusAllByLatest/-/${type}`)
            window.location.reload()
        }else if(type == "" ||type == "-"){
            history.push(`/cactusAllByLatest/${words}/-`)
            window.location.reload()
        }else{
            history.push(`/cactusAllByLatest/${words}/${type}`)
            window.location.reload()
        }
...
export default ListAllCactusComponentByLatest;