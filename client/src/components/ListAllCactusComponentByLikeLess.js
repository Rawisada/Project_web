...
const ListAllCactusComponentByLikeLess=(props)=>{
    ...
    const fetchData=()=>{
        axios.get(`${process.env.REACT_APP_API}/cactusByLikeLessToMore/${props.match.params.word}/${props.match.params.type}`)
        .then(response=>{
    ...
    for(let i=0; i < listCactus.length;i++){
        likeClass[i].onclick = function(){
            if(this.src === heart){
                ...
                axios.put(`${process.env.REACT_AhtPP_API}/pullcactus/${cactuss[i].slug}`, {user})
                .then(response=>{
                    console.log(cactuss[i].user)
                    axios.put(`${process.env.REACT_APP_API}/updateLike/${cactuss[i].slug}`, {like})
                    .then(response=>{
    ...
    function myFuntionSearch(element,words){
        ...
        if(words == "" && type == "" ){
            history.push('/cactusAllByLikeLess/-/-')
            window.location.reload()
        }else if(words == "" && type == "-"){
            history.push('/cactusAllByLikeLess/-/-')
            window.location.reload()
        }else if(words == ""){
            history.push(`/cactusAllByLikeLess/-/${type}`)
            window.location.reload()
        }else if(type == "" ||type == "-"){
            history.push(`/cactusAllByLikeLess/${words}/-`)
            window.location.reload()
        }else{
            history.push(`/cactusAllByLikeLess/${words}/${type}`)
            window.location.reload()
        }
...
export default ListAllCactusComponentByLikeLess;